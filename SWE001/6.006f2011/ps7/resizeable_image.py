...
#             print self.energy(i, self.height-1)
        (self.minSeamEnergy,self.minSeam) = self.findMinSeam(15,5)#(self.height-1))
        print self.minSeam
        ...
        if (i,j) in self.exploredPath.keys():  # already explored
            ...
            if j==0:
                ...
                if currentPath[-1][1] == j:    #this shouldn't happen
                    print "ERROR, path includes current row",currentPath,i,j    # how did a path this long end up here?
                    print potentialSubSeams
                currentPath.append((i,j))
                if len(currentPath) > j+1:    # this should happen before the other error
                    print "ERROR, path is longer than current height"
